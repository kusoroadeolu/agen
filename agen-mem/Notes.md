...
In a multiprocessor chip. The processor consists of multiple sequential core. Each core contains its own **private data cache**(which is *writeback*) and a shared LLC(last level cache) which is shared by all cores. All private caches and the LLC communicate through their cache controllers, over a shared intercommunication network
...
1. If a core reads from a memory location, a message is sent to other cores to re-read their values from that memory location, and also it also ends the current epoch if it's a read/write epoch
2. If a core writes to a memory location, a message is sent to other cores to re-read their values from that memory location, and also it also ends the current epoch if it's a read/write or a read only epoch
